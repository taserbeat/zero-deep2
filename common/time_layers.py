# coding: utf-8
import numpy as np
from common.layers import Embedding, SigmoidWithLoss, SoftmaxWithLoss, Affine
...
class RNN:
    ...
    def backward(self, dh_next):
        Wx, Wh, b = self.params
        x, h_prev, h_next = self.cache  # type: ignore
...
class TimeRNN:
    ...
    def backward(self, dhs):
        ...
        for t in reversed(range(T)):
            layer = self.layers[t]  # type: ignore
...
class LSTM:
    ...
    def backward(self, dh_next, dc_next):
        Wx, Wh, b = self.params
        x, h_prev, c_prev, i, f, g, o, c_next = self.cache  # type: ignore
...
class TimeAffine:
    ...
    def backward(self, dout):
        x = self.x
        N, T, D = x.shape  # type: ignore
        W, b = self.params

        dout = dout.reshape(N * T, -1)
        rx = x.reshape(N * T, -1)  # type: ignore

        db = np.sum(dout, axis=0)
        dW = np.dot(rx.T, dout)
        dx = np.dot(dout, W.T)
        dx = dx.reshape(*x.shape)  # type: ignore
...
class TimeSoftmaxWithLoss:
    ...
    def backward(self, dout=1):
        ts, ys, mask, (N, T, V) = self.cache  # type: ignore
...
class TimeDropout:
    ...
    def forward(self, xs):
        if self.train_flg:
            ...
            self.mask = flg.astype(np.float32) * scale  # type: ignore
...
class TimeSigmoidWithLoss:
    ...
    def backward(self, dout=1):
        N, T = self.xs_shape  # type: ignore
        dxs = np.empty(self.xs_shape, dtype='f')  # type: ignore
...
class GRU:
    ...
    def backward(self, dh_next):
        ...
        Wxz, Wxr, Wxh = Wx[:, :H], Wx[:, H:2 * H], Wx[:, 2 * H:]
        Whz, Whr, Whh = Wh[:, :H], Wh[:, H:2 * H], Wh[:, 2 * H:]
        x, h_prev, z, r, h_hat = self.cache  # type: ignore
...
class Simple_TimeSoftmaxWithLoss:
    ...
    def backward(self, dout=1):
        layers, xs = self.cache  # type: ignore
        N, T, V = xs.shape
